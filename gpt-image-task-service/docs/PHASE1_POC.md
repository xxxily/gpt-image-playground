...
- 增加真实 Hatchet adapter 入口：`npm run hatchet:worker`、`npm run hatchet:run`、`npm run hatchet:live-smoke`。这些命令在有 `HATCHET_CLIENT_TOKEN` 和可用 control plane 时注册并执行 mock task；无 token 时 `run` 与 `live-smoke` 安全跳过。
...
- 已实现真实 Hatchet worker/run/live-smoke adapter，但尚未在本机 control plane 注册 worker 并执行 mock task。
...
| Hatchet adapter 编译    | `rtk npm run hatchet:run`、`rtk npm run hatchet:live-smoke`  | 无 `HATCHET_CLIENT_TOKEN` 时安全跳过；可验证 adapter 入口能构建和启动到 env gate。                                  |
...
3. 运行 `npm run hatchet:worker` 或 `npm run hatchet:live-smoke`，把 mock task 注册到 Hatchet 并执行一次 live task。
