...
**分享按钮：** 输入框上方的“分享”按钮可以选择要写入链接的内容。默认包含提示词和模型，API Key 默认不包含；如果勾选 API Key，需要再次确认风险后才能复制。

**密码加密分享：** 勾选“使用密码加密整个分享链接”后，提示词、模型、API Base URL、API Key 和自动生成开关会被整体加密到 `sdata` 中，链接表面不会暴露明文参数。可以点击“随机”生成 8 位解密密码；默认密码不会写入链接或保存到浏览器，请通过其他可信渠道告诉接收者。如需更便捷转发，可以勾选“复制时附带解密密码”，生成 `?sdata=...#key=...` 形式的完整链接，接收者打开后会自动解密并清理地址栏中的密码片段。请注意：拿到这个完整链接的人也等同拿到了密码。
