...
| `sdata`                                         | 密码加密后的分享数据；接收者需要输入分享者另行提供的密码才能解密                      |
...
**密码加密分享：** 勾选“使用密码加密整个分享链接”后，提示词、模型、API Base URL、API Key 和自动生成开关会被整体加密到 `sdata` 中，链接表面不会暴露明文参数。密码不会写入链接或保存到浏览器，请通过其他可信渠道告诉接收者；建议使用随机生成、不可猜测的强密码，避免 `password`、连续数字或重复字符。
