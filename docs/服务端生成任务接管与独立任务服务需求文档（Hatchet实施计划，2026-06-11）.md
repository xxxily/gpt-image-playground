---
title: 服务端生成任务接管与独立任务服务需求文档（Hatchet 实施计划，2026-06-11）
...
canonicalDoc: ./requirements/SERVER_GENERATION_TASK_ORCHESTRATION_REQUIREMENTS.md
relatedDocs:
    - ./requirements/UNIFIED_PROVIDER_CONFIGURATION_REUSE_REQUIREMENTS.md
    - ./requirements/VIDEO_GENERATION_REQUIREMENTS_v2.md
    - ./requirements/BATCH_IMAGE_GENERATION_REQUIREMENTS.md
    - ./requirements/CLOUD_SYNC_IMPROVEMENT_AND_PROTOCOL_EXPANSION.md
    - ./requirements/PROJECT_HEALTH_REFACTOR_REQUIREMENTS.md
---

# 服务端生成任务接管与独立任务服务需求文档（Hatchet 实施计划，2026-06-11）

> 本文件是放在 `docs/` 根目录下的可见副本，便于后续 agent 直接按文档实施。规范源文件仍位于 `docs/requirements/SERVER_GENERATION_TASK_ORCHESTRATION_REQUIREMENTS.md`；实施阶段回写状态时，优先更新规范源文件，并保持本副本同步。
...
| ------- | ----------------------------------- | ----------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Phase 0 | 需求冻结与 Hatchet ADR              | Accepted    | 本文档评审通过   | [Phase 0 ADR](./requirements/SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md)、[API contract](./requirements/SERVER_GENERATION_TASK_SERVICE_API_CONTRACT.md) | 2026-06-11 已冻结 P0 范围、独立服务边界、Hatchet、状态映射、临时凭证和本地文件资产方案。                        |
...
| 子任务            | 状态     | 验收标准                                                                                                     | 备注                                                                                                                                                                              |
| ----------------- | -------- | ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 确认 P0 范围      | Accepted | ADR 明确 P0 只做 `image.generate` 与 `image.edit`，图生文、视频、批量、多租户列入后续。                      | 见 [Phase 0 ADR](./requirements/SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 1 节。                                                                                           |
| 固化服务边界      | Accepted | ADR 明确任务服务是独立项目、独立依赖、独立部署；当前 App 不 import 任务服务内部代码。                        | 见 [Phase 0 ADR](./requirements/SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 2 节。                                                                                           |
| 固化 Hatchet 选型 | Accepted | ADR 记录 Hatchet 版本、部署形态、依赖、授权、备选方案放弃原因。                                              | 见 [Phase 0 ADR](./requirements/SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 3 节；Phase 1 锁定运行版本。                                                                     |
| 状态机映射        | Accepted | 文档说明 Hatchet run/task 状态如何映射本文 `queued/running/provider_processing/succeeded/failed/cancelled`。 | 见 [Phase 0 ADR](./requirements/SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 4 节；不暴露 Hatchet 原生状态。                                                                  |
| 临时执行凭证方案  | Accepted | 文档说明用户本地 Key 与管理员 Key 如何变成短 TTL `executionCredential`，且不以明文持久化。                   | 见 [Phase 0 ADR](./requirements/SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 5 节和 [API contract](./requirements/SERVER_GENERATION_TASK_SERVICE_API_CONTRACT.md) 第 5.3 节。 |
| 资产存储方案      | Accepted | 文档说明 P0 使用任务服务本地文件系统，列出目录、URL、保留期、容量上限、清理任务；S3 为后续扩展。             | 见 [Phase 0 ADR](./requirements/SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 6 节。                                                                                           |
