...
以下 key 已预置到 `src/lib/i18n/messages.ts` 的 `video.*` 与 `settings.video.*` 命名空间，覆盖视频生成工作台所有用户可见文案。key 集在 `zh-CN` 与 `en-US` 之间完全一致，由 `messages.test.ts` 校验。

### 6.1 模式与工具栏 (spec §6.1, §6.2)

| 命名空间前缀 | 对应 UI 表面 |
|---|---|
| `video.mode.*` | 工作台模式切换标签（文生视频 / 图生视频）全称与简称 |
| `video.toolbar.*` | 工具栏按钮 label、tooltip、禁用态提示、活跃态标记 |

### 6.2 表单区 (spec §6.2)

| 命名空间前缀 | 对应 UI 表面 |
|---|---|
| `video.form.*` | 表单标题、提交按钮、placeholder、参考图片角色、源图片数量超限提示、模型/供应商未配置提示 |

### 6.3 参数面板 (spec §6.3)

| 命名空间前缀 | 对应 UI 表面 |
|---|---|
| `video.params.*` | 时长、比例、分辨率、画质等级、帧率、生成数量、负向提示词、种子、提示词增强、镜头运动、分镜模式、原生音频、水印的 label、描述和选项值 |

### 6.4 任务状态与输出面板 (spec §8.1, §9)

| 命名空间前缀 | 对应 UI 表面 |
|---|---|
| `video.status.*` | 任务状态标签（排队、运行中、轮询中、已完成、失败、已取消、过期） |
| `video.output.*` | 输出区占位文案、计时、进度条、操作按钮（取消、重试、重新生成、复制、下载、发送、保存）、URL 过期提醒 |

### 6.5 历史面板 (spec §10)

| 命名空间前缀 | 对应 UI 表面 |
|---|---|
| `video.history.*` | 历史 tab 标签、空状态、卡片类型、操作按钮（播放、删除、查看详情、恢复、重新生成、复制提示词、下载、发送首帧）、删除确认弹窗、批量删除、元数据（时长、分辨率、音频、源图数）、同步状态、可恢复任务提醒 |

### 6.6 设置面板 (spec §7.3)

| 命名空间前缀 | 对应 UI 表面 |
|---|---|
| `settings.video.*` | 视频生成设置页标题、描述、默认供应商/模型/时长/比例/画质、提示词增强、原生音频、保存历史、自动下载、轮询间隔、超时、重试次数、同步策略子面板 |

### 6.7 错误与通知 (spec §8.3, §14)

| 命名空间前缀 | 对应 UI 表面 |
|---|---|
| `video.error.*` | 供应商未配置、网络不可用、超时、内容被安全策略拒绝、源图片无效、公网 URL 要求、无法恢复轮询、聚合平台端点提醒 |

### 6.8 分享 (spec §13)

| 命名空间前缀 | 对应 UI 表面 |
|---|---|
| `video.share.*` | 分享时是否包含参数的开关、视频文件不含在分享链接中的提醒 |
