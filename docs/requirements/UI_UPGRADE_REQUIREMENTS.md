...
1. ✅ **主题语义统一**：5 张主卡片（generation-form / editing-form / image-output / text-output / history-panel）不再写 `text-white` / `bg-white` / `bg-black` / `text-white/xx` / `bg-white/[0.0x]` / 写死 hex。统一切换到语义 token（剩余 ~10 处都是品牌按钮 saturated bg 上的 `text-white` 或图片 thumbnail 上的 scrim，故意保留并已注释）。
2. 🔁 **去掉 globals.css 的浅色补丁层**：本轮保留。三大对话框（settings / share / prompt-templates）尚未迁移，patch 层删除会导致它们在浅色下显示坏掉。**留给 V2 完成。**
3. ✅ **首屏更简洁**：根 layout 的三个装饰色块（violet / blue / purple blur）已删除，网格背景保留并已是 5%（浅色）/ 10%（深色）。
4. ✅ **统一卡片骨架**：5 张主卡的 class 串提取到 `<WorkbenchCard>`，并把卡顶 1px 高光线移入 `app-panel-card::before` 自动渲染。
5. ✅ **补齐基础 primitives**：本轮新增 `IconButton`、`Spinner`、`Skeleton`、`EmptyState`、`WorkbenchCard` 共 5 个。`Popover` 和 `Heading` 推迟到 V2（Popover 需要新 npm 依赖、Heading 改造面积大）。
6. 🔁 **移动端可点击区域达标**：本轮未做。历史卡片图标按钮当前仍是 28×28 / 24×24，**留给 V2 用 IconButton primitive 替换实现 ≥ 40×40。**
7. 🔁 **一致的 hover/focus/active**：IconButton 默认带齐 `focus-visible:ring-[3px]` + `active:scale-[0.97]`，但旧组件（zoom-viewer 等）的 focus 状态待 V2 替换。
8. 🔁 **断点和媒体查询统一**：`src/lib/breakpoints.ts` 已提供工具，但 4 处旧字面量（page.tsx:352、promo-slot.tsx、editing-form.tsx:1832、prompt-templates-dialog.tsx:345）尚未替换，**留给 V2。**
9. ✅ **深浅主题双向验证**：V1 改动到的页面已在 4 种状态（桌面/移动 × 浅/深）实测，token 解析正确、无 console error、装饰元素 0 个。截图仅作开发调试用，未入库。
...
### 13.1 V2 范围确认（开工前 5 分钟）

- [ ] 决议 §4.2 Skeleton 实际接入列表：**历史卡片懒加载** / **模板列表** / **设置 promo 列表** / 暂不接入
- [ ] 决议 `<Heading>` primitive 是否在 V2 同步引入（影响 13 个 `<h1-h6>` 文件改造工作量）
- [ ] 决议 `<Popover>` 引入 `@radix-ui/react-popover` 依赖（增加约 30KB）

### 13.2 V2 推进顺序（按依赖关系）

1. 🔁 引入 `<Popover>`（依赖：history-panel 同步菜单、settings-dialog 折叠 section 都需要）
2. 🔁 `settings-dialog.tsx` token 化 + IconButton + Popover + 折叠 section 改 disclosure
3. 🔁 `share-dialog.tsx` token 化 + IconButton + 与 settings-dialog 头部对齐
4. 🔁 `prompt-templates-dialog.tsx` token 化 + IconButton + 头部对齐 + hex 颜色清理
5. 🔁 `zoom-viewer.tsx` 改用 `<IconButton variant=overlay>`，加 focus-visible
6. 🔁 `history-panel.tsx` 内联 button → IconButton（标签 / 同步菜单 / 卡片操作），移动端尺寸 ≥ 40×40
7. 🔁 替换 `Loader2 + animate-spin` → `<Spinner>`（14 处）
8. 🔁 `<EmptyState>` 接入 `image-output` / `text-output` / 历史空状态 / 模板空筛选
9. 🔁 4 处 viewport 字面量 → `BREAKPOINTS` / `useIsMobileViewport`
10. 🔁 admin 移动端响应式（表格转卡片，移动端 sidebar 入口）
11. 🔁 删除 `globals.css` 行 182–275 浅色补丁层（**前置**：1-9 步必须全部完成）
12. 🔁 4 种状态全量截图回归 + lint/test/build + Tauri desktop 验证

### 13.3 V2 退出标准

参考 §9 表格的"V2 目标"列，全部达成才能视为 V2 完成。
