...
```typescript
type AdditionalVideoProviderKind = 'xai';

type AdditionalVideoProviderProtocol = 'xai-imagine-video';
```
...
4. Base URL 仍需走现有规范化和协议校验，但模型发现可以接受 localhost、私网地址和局域网网关；不再把这些地址当作刷新模型的拦截条件。
