...
summary: 将图片生成/编辑、图生文、多模态文本、提示词润色、视频等分散配置，收敛为 endpoint-first 的供应商与模型主数据体系；任务页只消费统一目录中的能力模型，不再各自维护凭证孤岛。
createdAt: 2026-05-23
updatedAt: 2026-05-23
status: reviewed-draft-requirement
---
...
1. 必须明确唯一主数据。`providerEndpoints`、`modelCatalog`、`modelTaskDefaultCatalogEntryIds` 是最终写入结构；旧字段只允许在一次性导入阶段读取。
2. 必须明确不做长期兼容。当前用户规模不大，本次可以做破坏性重构，但要提供一次性导入、导出前提示和重置路径，避免把旧心智永久保留下来。
...
本文在保留原目标的基础上，把需求整理成可以直接拆任务、验收和实现的版本。
...
- 提示词润色仍有 `polishingApiKey`、`polishingApiBaseUrl`、`polishingModelId` 等独立入口。
...
这些字段只允许在迁移阶段读取一次，或作为环境变量/服务端兜底读取，不应继续作为用户日常配置入口。

### 3.2 可以做破坏性重构，但要可恢复

当前项目使用人数不多，本次可以不做长期向后兼容。推荐策略：

1. 首次打开新版配置时，把旧字段导入到统一主数据。
2. 导入后保存只写统一主数据，不再主动双写旧字段。
3. 旧 UI 不再作为主路径展示，最多保留一个短期“导入旧配置”工具。
4. 导入前允许用户导出当前配置 JSON。
5. 如果导入失败，保留原始 JSON 备份并提示用户重新导入或重置。

这比长期兼容更合理：既不给少量老用户制造不可恢复的损失，也避免代码永久背两套配置心智。
...
| 入口 | 定位 | 主要内容 |
| --- | --- | --- |
| 供应商与模型 | 主入口 | 端点、Key、Base URL、协议、发现模型、已选模型、能力标注、使用情况 |
...
| 提示词润色 | 任务设置 | 润色默认模型、批量规划模型、思考参数、润色预设、自定义润色提示词 |
| 视频任务默认值 | 任务设置 | 文生视频/图生视频默认模型、时长、比例、分辨率、轮询、重试 |
| 模型能力检视 | 高级工具 | 跨端点搜索、能力审计、低置信度模型定位、跳转到端点模型详情 |
| 运行与存储 | 环境设置 | 连接模式、并发数、历史、存储、同步、桌面代理 |

`统一模型目录` 不再作为与 `供应商与模型` 并列的主入口。它应下沉为 `供应商与模型` 内的“模型能力检视”高级区。
...
由于本次不要求长期向后兼容，推荐“一次性导入 + 新结构独写”。

流程：

1. 检测到旧字段存在且统一主数据缺失或版本较旧。
2. 展示“将旧供应商配置导入到新版供应商与模型”的提示。
3. 允许用户先导出当前配置 JSON。
4. 执行导入：
   - 图片供应商导入为统一端点。
...
如果某些执行链路暂时还没有完成统一改造，应先改执行链路，而不是继续双写旧字段。
...
| 润色模型 ID | 提示词润色 > 默认模型 | `modelTaskDefaultCatalogEntryIds['prompt.polish']` |
| 批量规划模型 | 提示词润色 > 默认模型 | `modelTaskDefaultCatalogEntryIds['prompt.batchPlan']` |
