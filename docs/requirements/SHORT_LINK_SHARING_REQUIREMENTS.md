...
- `#key=`：允许随加密分享存入短链目标，但必须记录风险标记；说明“短链数据库拿到完整目标 URL 的人也等同拿到解密密码”。
...
由于 `#key=` 等同解密密码，短链保存它时必须记录风险标记，并在用户文档中提示短链目标 URL 的持有者等同持有解密密码。
...
- `#key=` 目标允许创建，并记录 inline-password 风险。
- 无效短码大量访问触发频控。
