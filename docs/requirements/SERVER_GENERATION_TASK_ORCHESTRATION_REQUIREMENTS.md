...
| Phase 6 | P0 验收与故障演练                   | Accepted    | Phase 4、Phase 5 | P0 验收报告、压测/断线/worker crash/fallback 证据                                                                               | 2026-06-11 已完成 P0 故障演练和验收报告；Phase 7 仍需单独立项。                                                 |
...
| 子任务            | 状态     | 验收标准                                                                                                     | 备注                                                                                                                                                    |
| ----------------- | -------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 确认 P0 范围      | Accepted | ADR 明确 P0 只做 `image.generate` 与 `image.edit`，图生文、视频、批量、多租户列入后续。                      | 见 [Phase 0 ADR](./SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 1 节。                                                                              |
| 固化服务边界      | Accepted | ADR 明确任务服务是独立项目、独立依赖、独立部署；当前 App 不 import 任务服务内部代码。                        | 见 [Phase 0 ADR](./SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 2 节。                                                                              |
| 固化 Hatchet 选型 | Accepted | ADR 记录 Hatchet 版本、部署形态、依赖、授权、备选方案放弃原因。                                              | 见 [Phase 0 ADR](./SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 3 节；Phase 1 锁定运行版本。                                                        |
| 状态机映射        | Accepted | 文档说明 Hatchet run/task 状态如何映射本文 `queued/running/provider_processing/succeeded/failed/cancelled`。 | 见 [Phase 0 ADR](./SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 4 节；不暴露 Hatchet 原生状态。                                                     |
| 临时执行凭证方案  | Accepted | 文档说明用户本地 Key 与管理员 Key 如何变成短 TTL `executionCredential`，且不以明文持久化。                   | 见 [Phase 0 ADR](./SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 5 节和 [API contract](./SERVER_GENERATION_TASK_SERVICE_API_CONTRACT.md) 第 5.3 节。 |
| 资产存储方案      | Accepted | 文档说明 P0 使用任务服务本地文件系统，列出目录、URL、保留期、容量上限、清理任务；S3 为后续扩展。             | 见 [Phase 0 ADR](./SERVER_GENERATION_TASK_SERVICE_PHASE0_ADR.md) 第 6 节。                                                                              |

阶段验收：已通过。ADR 与 API contract 已落地；P0 范围、Hatchet、Key 委托、资产存储、角色可见性和自动重试策略都无待确认项。Phase 1 可以创建独立任务服务骨架并做 Hatchet mock 任务 PoC。
...
| 子任务             | 状态     | 验收标准                                                                       | 备注                                                                                                                                                                                                                                                           |
| ------------------ | -------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 创建独立项目       | Accepted | 新项目可独立安装依赖、启动、测试、构建；不依赖当前 App runtime。               | 已创建 `gpt-image-task-service/`，具备独立 `package.json`、锁文件、`tsconfig.json`、`Dockerfile`、README、源码和测试；未接入根项目 workspace。                                                                                                                 |
| 接入 Hatchet       | Accepted | 本地或测试环境可连接 Hatchet control plane，worker 可注册并执行 mock task。    | 已锁定 `@hatchet-dev/typescript-sdk@1.23.1`，新增真实 Hatchet mock task、worker、run、live-smoke 和 live-verify adapter；通过本地 Hatchet profile 注入环境执行 `hatchet:probe`、`hatchet:live-smoke`、`hatchet:live-verify`，worker 可注册并完成 mock task。   |
| 实现 mock 图片任务 | Accepted | mock `image.generate` 与 `image.edit` 支持延迟、进度、失败、取消、成功结果。   | mock runtime 与 HTTP API 已覆盖 queued/running/provider_processing/downloading_result/succeeded/failed/cancelled/retry_scheduled。                                                                                                                             |
| 验证 worker crash  | Accepted | 运行中终止 worker 后，任务可恢复或进入明确 retry/failed 状态，不丢失任务记录。 | 领域 mock runtime 已验证任务记录保留和 retry_scheduled；`hatchet:live-verify` 使用 `crashOnceKey` 触发 worker 退出码 42，Hatchet run 崩溃后为 `QUEUED`，替换 worker 启动后最终 `COMPLETED`，结果 `recovered: true`。                                           |
| 验证端点级限流     | Accepted | 不同 `baseUrlFingerprint` 可应用不同并发或速率策略。                           | 领域层 `EndpointLimiter` 已按 `baseUrlFingerprint` 分桶限并发；Hatchet adapter 声明 `input.endpointFingerprint` 维度 concurrency 与 dynamic rate limit；`hatchet:live-verify` 证明同一 endpoint 两个任务 `overlapped: false`、`sameEndpointSerialized: true`。 |

阶段验收：已通过。独立服务可安装、构建、测试、启动并通过 HTTP 完成 mock 图片任务生命周期；取消、失败、手动重试、mock worker crash 和端点级并发有测试或演示记录；真实 Hatchet adapter 已编译并提供 worker/run/live-smoke/live-verify 入口，且测试证明 Hatchet payload 不包含 credential、原始 prompt、provider URL 或输入资产。`hatchet:live-smoke` 已完成真实 worker 注册和 mock task 执行；`hatchet:live-verify` 已证明真实 Hatchet 执行、同 endpoint 并发串行和 worker crash 后替换 worker 恢复。Phase 1 标记为 `Accepted`，可进入 Phase 2。
...
| 子任务              | 状态     | 验收标准                                                                        | 备注                                                                                                    |
| ------------------- | -------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 任务服务配置模型    | Accepted | 可保存服务 URL、鉴权、启用状态、健康检查配置、capabilities 摘要。               | 已新增当前 App SQLite 表、admin API 和 UI；Bearer Token 用 AES-GCM 加密保存，前端只返回配置状态和前缀。 |
| 端点接管策略模型    | Accepted | 支持按 `ProviderEndpoint.id`、normalized `apiBaseUrl`、模型、任务类型匹配。     | 策略模型支持端点、Base URL、provider kind/protocol、模型目录 ID 和 P0 图片任务类型。                    |
| resolver            | Accepted | 对未命中策略的端点保持现有 direct/proxy 行为；命中后选择 `managed-task`。       | `resolveManagedTaskExecution` 覆盖未命中、unsupported、特异性优先级、服务缺失/停用/不可用 fallback。    |
| 后台 UI 与 i18n     | Accepted | 新增文案覆盖所有支持语言；浅色/深色、移动/桌面布局可用。                        | 新增 `/admin/managed-tasks`；中英文 i18n 已同步；Playwright 验证桌面/移动、浅色/深色无横向溢出。        |
| 健康检查与 fallback | Accepted | 任务服务不可用时按策略 fail-closed、ask-user、proxy 或 direct；默认不悄悄回退。 | 健康检查仅调用任务服务 HTTP `/v1/admin/health` 与 `/v1/admin/capabilities`；不可用时按策略 fallback。   |

阶段验收：已通过。后台可以配置任务服务和端点接管策略；测试连接、capabilities 读取、策略命中展示和 fallback 行为都有测试。Phase 3 代码提交 `14f97ee feat: add managed task admin configuration`；验证通过 `rtk npm run typecheck`、`rtk npm test -- src/lib/managed-task-config.test.ts src/lib/server/managed-task-admin.test.ts src/lib/i18n/messages.test.ts src/lib/i18n/i18n-audit.test.ts`、`rtk npm run lint`、`rtk npm run build`、Prettier check、`rtk git diff --check` 和 Playwright 桌面/移动、浅色/深色检查。
...
| 子任务            | 状态     | 验收标准                                                                           | 备注                                                                                                                                            |
| ----------------- | -------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| managed-task 提交 | Accepted | 被接管端点的图片生成/编辑提交到任务服务，立即返回稳定 `managedTaskId`。            | Web 端通过当前 App `/api/managed-tasks/*` facade 提交；用户本地 Key 在服务端封装成短 TTL `sealed-box-v1` 执行凭证，不向任务服务发送明文。       |
| 本地 pending 记录 | Accepted | 保存 `managedTaskId`、endpoint、model、workspace、任务服务 ID、导入状态等。        | 新增浏览器 localStorage pending record，记录 prompt/参数摘要、workspace、batch metadata 和导入状态，不保存 raw key。                            |
| 状态同步          | Accepted | 页面启动、网络恢复、前台恢复时批量查询状态，显示 queued/running/failed/succeeded。 | 启动、online、focus、visibility resume 会批量查询；单任务轮询在同 tick 合并，避免 N 个任务 N 个请求；query/result 继续校验 App password hash。  |
| 结果导入          | Accepted | `succeeded` 后读取 manifest，将图片结果导入历史和当前工作空间。                    | 只允许从任务服务同 origin 下载结果 URL；导入后写入当前图片处理/历史路径，并更新 pending record 为 `imported`。                                  |
| 旧路径回归        | Accepted | 未开启接管的 direct/proxy 图片生成和编辑行为不变。                                 | 未命中 resolver 或 resolver 不可用时回退旧 direct/proxy；Tauri desktop 新提交显式跳过 Web API managed-task 路径，继续使用旧 direct/Rust proxy。 |

阶段验收：已通过。Web 用户提交接管任务后可拿到稳定 `managedTaskId`，刷新/恢复后可批量同步 pending task 并导入结果；运行详情展示服务端任务接管、任务服务、服务端任务 ID 和状态；未开启接管的 direct/proxy 路径保持旧行为。Phase 4 代码提交 `f3ad7d4 feat: add managed task user submission recovery`。验证覆盖 typecheck、相关单测、lint、build、Prettier/diff 检查，以及 Web 桌面/移动浅色/深色浏览器场景。Tauri 原生 managed-task 新提交仍不在本阶段启用，避免静态导出依赖 Next API Route。
...
| 子任务         | 状态     | 验收标准                                                          | 备注                                                                                                                                           |
| -------------- | -------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 断线恢复       | Accepted | 提交任务后关闭页面/断网，恢复后能同步状态和结果。                 | Web 移动 390x844 验证通过；`online` 恢复和页面刷新恢复均将 pending record 更新为 `succeeded/imported`，历史条目数为 2。                        |
| worker crash   | Accepted | worker 停止/重启后任务不丢失，不重复已完成付费调用。              | `phase6:drill` 验证 crash 后任务保留并以 attempt 2 成功；真实 Hatchet crash/replacement 证据沿用 Phase 1 `hatchet:live-verify`。               |
| 队列压力       | Accepted | 100 个图片任务批量提交时，当前 App 首页、历史、非接管任务仍可用。 | `phase6:drill` 提交 100 个 mock 图片任务，端点 limit 2 下初始 active 2、queued 98，最终 100 个 `succeeded`；当前 App lint/build/测试不受影响。 |
| 供应商 429/5xx | Accepted | 触发端点级退避/熔断，任务状态可解释，自动重试遵守配置。           | mock provider 429/5xx 均进入 `retry_scheduled`，自动重试遵守 `maxAttempts/backoffMs`，最终 attempt 2 成功并记录 2 条 `task_retry_scheduled`。  |
| 任务服务不可用 | Accepted | 当前 App 短超时和断路器生效，按 fallback 处理，不拖垮其他业务。   | `task_service_unavailable` resolver 结果会回退 `/api/images`，不会调用 `/api/managed-tasks/submit`；测试覆盖 Web proxy fallback。              |
| 验收报告       | Accepted | 产出 P0 验收报告，记录命令、截图、风险、未覆盖项。                | 已更新 `docs/agent-reports/2026-06-10-server-generation-task-orchestration-requirements.md`，记录命令、浏览器场景、风险和 Phase 7 边界。       |

阶段验收：已通过。Phase 6 代码提交 `6dbb069 feat: add managed task phase 6 fault drills`；验证覆盖 `rtk npm run typecheck`、相关单测、`rtk npm run lint`、`rtk npm run build`、`rtk npm test --prefix gpt-image-task-service`、`rtk npm run phase6:drill --prefix gpt-image-task-service`、`rtk npm run smoke --prefix gpt-image-task-service` 和 Web 移动断线/刷新恢复脚本。Phase 7 仍为非 P0 后续增强，需单独立项。
