...
| 接入 Hatchet       | Partial  | 本地或测试环境可连接 Hatchet control plane，worker 可注册并执行 mock task。    | 已锁定 `@hatchet-dev/typescript-sdk@1.23.1` 并验证 SDK 可加载；已安装 Hatchet CLI `0.89.0` 到用户目录；`hatchet server start` 因 Docker daemon 不可用失败，未获得本地 profile/token，未注册真实 worker。 |
...
| 验证端点级限流     | Partial  | 不同 `baseUrlFingerprint` 可应用不同并发或速率策略。                           | 已实现并测试领域层 `EndpointLimiter` 按 `baseUrlFingerprint` 分桶限并发；Hatchet dynamic concurrency/rate limit 映射未完成真实控制面验证。                                                               |

阶段验收：部分通过。独立服务可安装、构建、测试、启动并通过 HTTP 完成 mock 图片任务生命周期；取消、失败、手动重试、mock worker crash 和端点级并发有测试或演示记录。阶段未达到 `Accepted`，因为本机 Docker daemon 不可用导致 Hatchet control plane 未启动，真实 Hatchet worker 注册、执行、崩溃恢复和动态限流映射没有验收证据。
