...
| Phase 1 | 独立任务服务骨架与 Hatchet 接入 PoC | Partial     | Phase 0          | [独立任务服务骨架](../../gpt-image-task-service/README.md)、[Phase 1 PoC 记录](../../gpt-image-task-service/docs/PHASE1_POC.md) | 2026-06-11 已完成 mock 服务闭环；Docker daemon 不可用，真实 Hatchet control plane/worker 验证未完成。 |
...
| 接入 Hatchet       | Partial  | 本地或测试环境可连接 Hatchet control plane，worker 可注册并执行 mock task。    | 已锁定 `@hatchet-dev/typescript-sdk@1.23.1`，新增真实 Hatchet mock task、worker、run、live-smoke adapter，并验证无 token 时安全跳过；已安装 Hatchet CLI `0.89.0` 到用户目录；`hatchet server start` 因 Docker daemon 不可用失败，未获得本地 profile/token，未完成真实 worker 注册/执行。 |
...
| 验证 worker crash  | Partial  | 运行中终止 worker 后，任务可恢复或进入明确 retry/failed 状态，不丢失任务记录。 | 已通过领域 mock runtime 的 `/v1/admin/test/simulate-worker-crash` 验证任务记录保留和 retry_scheduled；缺少真实 Hatchet worker 崩溃恢复证据。                                                                                                                                             |
| 验证端点级限流     | Partial  | 不同 `baseUrlFingerprint` 可应用不同并发或速率策略。                           | 已实现并测试领域层 `EndpointLimiter` 按 `baseUrlFingerprint` 分桶限并发；Hatchet adapter 已声明 `input.endpointFingerprint` 维度的 concurrency 与 dynamic rate limit，真实控制面映射仍未完成 live 验证。                                                                                 |

阶段验收：部分通过。独立服务可安装、构建、测试、启动并通过 HTTP 完成 mock 图片任务生命周期；取消、失败、手动重试、mock worker crash 和端点级并发有测试或演示记录；真实 Hatchet adapter 已编译并提供 worker/run/live-smoke 入口，且测试证明 Hatchet payload 不包含 credential、原始 prompt、provider URL 或输入资产。阶段未达到 `Accepted`，因为本机 Docker daemon 不可用导致 Hatchet control plane 未启动，真实 Hatchet worker 注册、执行、崩溃恢复和动态限流映射没有 live 验收证据。
