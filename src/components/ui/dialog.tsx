  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[calc(50%+((max(1rem,env(safe-area-inset-top))-max(1rem,env(safe-area-inset-bottom)))/2))] left-[50%] z-50 grid max-h-[calc(100dvh-max(1rem,env(safe-area-inset-top))-max(1rem,env(safe-area-inset-bottom)))] w-[calc(100vw-1rem)] max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 overflow-y-auto overscroll-contain rounded-lg border p-6 pb-[max(1.5rem,env(safe-area-inset-bottom),var(--app-keyboard-inset-bottom,0px))] shadow-lg duration-200 sm:top-[50%] sm:max-h-[calc(100dvh-2rem)] sm:w-full",
          className
        )}
        {...props}
      >
