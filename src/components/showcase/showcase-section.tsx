...
import { ArrowRight, ChevronDown, Compass, RefreshCw } from 'lucide-react';
...
export function ShowcaseSection({ catalog: catalogOverride, className, maxTopics = 3 }: ShowcaseSectionProps) {
    ...
    React.useEffect(() => {
        if (catalogOverride || shouldLoad || !expanded) return;
    ...
    }, [catalogOverride, expanded, shouldLoad]);
    ...
    React.useEffect(() => {
        ...
        observer.observe(node);
        return () => observer.disconnect();
    }, [expanded, state.catalog.catalogRevision, topics]);

    return (
        <section
            ref={sectionRef}
            className={`app-panel-card min-w-0 rounded-xl border p-3 shadow-sm sm:p-4 ${className ?? ''}`}
            aria-labelledby='showcase-section-title'>
            <div className='flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between'>
                <div className='flex min-w-0 items-start gap-3'>
                    <span className='bg-panel-subtle text-on-panel-muted mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-lg border'>
                        <Compass className='size-4' aria-hidden='true' />
                    </span>
                    <div className='min-w-0'>
                        <h2 id='showcase-section-title' className='text-foreground text-base font-semibold sm:text-lg'>
                            {t('showcase.section.title')}
                        </h2>
                        <p className='text-on-panel-muted mt-0.5 line-clamp-2 text-xs leading-5 sm:text-sm'>
                ...
                <div className='flex shrink-0 items-center justify-end gap-1'>
                    ...
                    <Button
                        ...
                        aria-controls='showcase-section-content'
                        ...
                        <ChevronDown
                            ...
                            aria-hidden='true'
                        />
                        {expanded ? t('showcase.section.collapse') : t('showcase.section.expand')}
                    </Button>
                </div>
            </div>

            {expanded ? (
                <div id='showcase-section-content' className='border-panel-divider mt-4 border-t pt-4'>
                    {topics.length > 0 ? (
