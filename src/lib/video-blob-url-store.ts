...
import { isBlobObjectUrl } from '@/lib/object-url';

type Listener = () => void;

const FLUSH_BATCH_SIZE = 16;

class VideoBlobUrlStore {
    ...
    private async loadBatch(filenames: string[]): Promise<void> {
        ...
        try {
            records = await db.videoBlobs.bulkGet(filenames);
        } catch (error) {
            console.warn('videoBlobUrlStore: bulkGet failed, falling back per-item', error);
...
export const videoBlobUrlStore = new VideoBlobUrlStore();
